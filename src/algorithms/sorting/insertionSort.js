...
export function generateInsertionSortSnapshots(initialArray) {
  ...
  record(StepTypes.START, [], [], "Starting Insertion Sort");
  ...
  for (let i = 1; i < n; i++) {
    ...
    record(StepTypes.PIVOT, [i], settled, `Selecting ${key} to insert into the sorted portion`);

    while (j >= 0) {
      record(StepTypes.COMPARE, [j, j + 1], settled, `Comparing ${key} with ${arr[j]}`);
      
      if (arr[j] > key) {
        // Shift right
        arr[j + 1] = arr[j];
        record(StepTypes.SWAP, [j, j + 1], settled, `Shifting ${arr[j]} to the right`);
        j--;
      } else {
        break;
      }
    ...
    arr[j + 1] = key;
    if (j + 1 !== i) {
      record(StepTypes.SWAP, [j + 1], settled, `Inserting ${key} into its correct position`);
    }
    ...
    record(StepTypes.SETTLED, [], settled, `The first ${i + 1} elements are sorted`);
  }
