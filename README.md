...
**Specimen** is a highly interactive, brutalist-style Algorithm Visualizer. It provides a "lil algorithm lab" environment where developers and students can observe complex algorithms executing step-by-step in real-time. 
...
![Specimen UI](src/assets/hero.png) *(Note: Add a screenshot of your app here!)*

---

## Key Features

- **🎨 Brutalist UI Design:** A clean, high-contrast interface designed for maximum readability.
...
- **✋ Interactive Data Structures:** Drag-and-drop graph nodes, draw walls/mud on grids, and edit edge weights in real-time without breaking the playback state.
...
| :--- | :--- | :--- |
| **Sorting** | Quick Sort, Merge Sort, Heap Sort, Radix Sort, Insertion Sort, Selection Sort, Shell Sort | Array Bar Chart |
...
│   ├── graph/        
│   └── backtracking/ 
├── components/       # Reusable UI components (Sidebar, PlaybackControls, ChatbotWidget)
...
├── views/            # Major visual layouts (GraphView, PathfindingView, SortingView, BacktrackingView)
...
2. It calls `yield { type: StepTypes.COMPARE, activeNodes: [...], message: "..." }`.
3. The React hook captures this snapshot and stores it in an array.
4. The UI simply renders whatever snapshot is currently selected by the timeline scrubber.
...
1. **Select an Algorithm:** Use the left sidebar to navigate categories and select a specific algorithm.
...
3. **Control Time:** Use the Playback Controls at the bottom of the screen to Play, Pause, Step Forward/Backward, or adjust the execution speed.
4. **View Pseudocode:** Click the `< / > show pseudocode` button to toggle the logic panel.
